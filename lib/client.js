// dsh-agents-roles — client half (static bundle form).
//
// Registers with the DSH web module loader and hooks the same chat/settings
// slots as the dynamic form: a "Roles" settings section, a read-only roles
// ladder pill in the composer (mirroring the standard model-selector UI), an
// auto-vision admission switch while images are pending, and a per-response
// model badge. Host state is reached through the package's `roles/*` Remote
// endpoints over the /api RPC channel (connection.rpc.call), not a dynamic
// runner.
//
// Plain hand-authored JavaScript: React via require("react"), timers via the
// Cordis `timer` service, styles via a managed <style> tag.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function injectStyles(css) {
      ...
      var tagId = "dsh-agents-roles/styles";
      if (document.querySelector('style[data-plugin-css="' + tagId + '"]')) return;
      var tag = document.createElement("style");
      tag.dataset.plugin = "dsh-agents-roles";
...
`);
    ...
    exports.apply = (ctx) => {
      ...
      // Client -> host bridge: the package's `roles/*` Typert Remote
      // endpoints over the generic /api RPC channel.
      async function rpc(method, args) {
        const conn = ctx.get('connection');
        if (!conn || !conn.rpc || typeof conn.rpc.call !== 'function') {
          throw new Error('roles: connection unavailable');
        }
        const result = await conn.rpc.call('/api', method, { args: args || {} });
        if (!result || result.ok !== true) {
          const err = result && result.error;
          throw new Error((err && err.message) || ('roles rpc failed: ' + method));
        }
        return result.value;
      }
      ...
      // highlighted) — the user cannot select models or roles here. With roles
      ...
      function RolesModelSelect(props) {
        ...
        const refresh = React.useCallback(() => {
          if (!sessionId) return;
          rpc('roles/getSessionRole', { sessionId }).then((r) => { if (r) setRole(r); }).catch(() => {});
          rpc('roles/getState').then((s) => {
            if (!s) return;
        ...
        if (!cfg) return el('span', { className: 'roles-composer-slot' }, '…');

        if (!cfg.enabled) {
          if (!models) return el('span', { className: 'roles-composer-slot' }, '…');
          ...
          if (!opts.length) opts.push(el('option', { key: '', value: '' }, '— no models —'));
          return el('select', {
        ...
        const triggerLabel = currentTier
          ? (currentTier.provider && currentTier.model
              ? currentTier.id + ' · ' + currentTier.provider + '/' + currentTier.model + (currentTier.reasoningEffort ? ' · ' + currentTier.reasoningEffort : '')
        ...
        return el('span', { className: 'roles-pill-root' },
          el('button', {
            ...
            title: triggerLabel + ' — roles ladder (read-only)',
            disabled: locked,
            onClick: () => setOpen((o) => !o),
          },
            el('span', { className: 'roles-pill-label' }, triggerLabel),
            el('span', { className: 'roles-pill-chevron' + (open ? ' roles-pill-open' : '') }, '▾'),
          ),
              ...
              tiers.length
                ? tiers.map((t) => el('div', {
                    ...
                    el('div', { className: 'roles-pill-row-main' },
                      ...
                      el('span', { className: 'roles-pill-row-desc' }, t.provider && t.model ? t.provider + '/' + t.model + (t.reasoningEffort ? ' · ' + t.reasoningEffort : '') : 'not configured'),
                    ),
                    t.id === currentRole ? el('span', { className: 'roles-pill-check' }, '✓') : null,
                  ))
                : el('div', { className: 'roles-pill-empty' }, 'no roles configured'),
            ),
            el('div', { className: 'roles-pill-note' }, 'Auto-routed by roles — read-only'),
          ) : null,
        );
      }
      ...
      function RolesVisionSwitch(props) {
        ...
        React.useEffect(() => {
          let cancelled = false;
          if (!sessionId) return;
          rpc('roles/visionModel', { sessionId }).then((res) => { if (!cancelled) setVision(res); })
            .catch(() => {});
      ...
      function RolesModelBadge(props) {
        ...
        React.useEffect(() => {
          ...
          rpc('roles/getStepModel', { sessionId, messageId }).then((res) => {
        ...
        return el('span', {
          ...
          title: (info.role ? 'role: ' + info.role + ' — ' : '') + 'model that produced this response',
        ...
        return el('div', { className: 'roles-overlay', onClick: (e) => { if (e.target === e.currentTarget) onClose(); } },
          el('div', { className: 'roles-modal' },
            el('div', { className: 'roles-modal-head' },
              ...
              el('button', { className: 'roles-btn', onClick: onClose }, '✕'),
            ),
            el('div', { className: 'roles-modal-body' },
              ...
              warnings.length
                ? el('div', { className: 'roles-result-warnings' },
                    ...
                    el('div', { className: 'roles-result-message' }, warnings.map((w, i) => el('div', { className: 'roles-result-warn', key: i }, el('span', null, '⚠'), el('span', null, w)))),
                  )
                : null,
            ),
      ...
      function RoleEditor(props) {
        ...
        React.useEffect(() => {
          ...
          let cancelled = false;
          rpc('roles/listModels', { provider }).then((res) => { if (!cancelled) setModels((res && res.models) || []); })
        ...
        React.useEffect(() => {
          ...
          rpc('roles/getModelInfo', { provider, model }).then((res) => {
            if (cancelled) return;
        ...
        let effortControl;
        if (effortLoading) {
          effortControl = el('span', { className: 'roles-hint' }, 'probing model…');
        } else if (efforts.length) {
          const opts = [el('option', { key: '__inherit', value: '' }, '— default —')];
        ...
        return el('div', { className: 'roles-overlay', onClick: (e) => { if (e.target === e.currentTarget) onCancel(); } },
          el('div', { className: 'roles-modal' },
            el('div', { className: 'roles-modal-head' },
              el('span', { className: 'roles-modal-title' }, role.id ? 'Edit role — ' + role.id : 'New role'),
              el('button', { className: 'roles-btn', onClick: onCancel }, '✕'),
            ),
            el('div', { className: 'roles-modal-body' },
      ...
      function CategoriesEditor(props) {
        ...
        return el('div', { className: 'roles-overlay', onClick: (e) => { if (e.target === e.currentTarget) onCancel(); } },
          el('div', { className: 'roles-modal' },
            el('div', { className: 'roles-modal-head' },
              el('span', { className: 'roles-modal-title' }, 'Categories — need → role'),
              el('button', { className: 'roles-btn', onClick: onCancel }, '✕'),
            ),
            el('div', { className: 'roles-modal-body' },
              el('div', { className: 'roles-card' },
                rows.length
                  ? rows.map((r, i) => el('div', { className: 'roles-row', key: i },
                      ...
                      el('button', { className: 'roles-btn danger', onClick: () => removeRow(i) }, '✕'),
                    ))
                  : el('span', { className: 'roles-hint' }, 'no categories yet — add one below'),
      ...
      function RoleCard(props) {
        ...
        const summary = ok
          ? tier.provider + '/' + tier.model + (tier.reasoningEffort ? ' · effort ' + tier.reasoningEffort : '')
      ...
      function RolesSection(ownerProps) {
        ...
        const reload = React.useCallback(() => {
          rpc('roles/getState').then((s) => { setConfig(s); setLoadError(''); setEditing(null); setCatEditing(false); setResult(null); })
        ...
        if (!config) {
          return el('div', { className: 'roles-settings' },
            loadError ? el('div', { className: 'roles-load-error' }, loadError) : el('div', { className: 'roles-label' }, 'Loading roles…'),
          );
        }
        ...
        return el('div', { className: 'roles-settings' },
          el('div', { className: 'roles-row' },
            ...
            config.workflowEngine
              ...
              : null,
            config.visionRole
              ? el('span', { className: 'roles-hint' }, 'images → ' + config.visionRole)
              : null,
