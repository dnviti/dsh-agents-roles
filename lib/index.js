...
export function apply(ctx) {
  ...
  // `dsh-agents-roles-boot.log` in the harness cwd when the fs service is up,
  // so a failing registration is visible instead of being swallowed by the
  // per-section catch blocks.
  const bootLog = [];
  ...
  try {
    ...
    if (fs && fs.resolve && fs.writeText) {
      fs.resolve('dsh-agents-roles-boot.log').then((p) => fs.writeText(p, bootLog.join('\n') + '\n')).catch(() => {});
    }
