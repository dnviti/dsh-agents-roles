// dsh-agents-roles — host half (static bundle form).
//
// Loaded as a profile bundle layer by `dsh plugin add` (see package.json
// `dsh.bundle.patch` + cordis.patch.yml). Runs inside the Harness host at
// boot with the full Cordis context: it registers the llm/agent waterfalls
// (role routing + ladder escalation), the session/workflow/subagent hooks
// (auto vision/category routing, workflow @role tags), a Typert Remote
// service (`roles/*` endpoints served over the /api RPC channel for the
// browser client), and the roles_* model tools.
//
// No build step and NO package imports: like every hand-authored out-of-tree
// DSH bundle, this module is plain ESM JavaScript with an empty import
// surface, so it resolves from any profile node_modules (Node resolves a
// bundle's own imports from its real location, which would miss the
// installation closure). The Remote service and the tools are wired through
// plain objects + ctx.provide / ctx.tools.register instead of importing
// dsh-typert-protocol / dsh-tools.

export const inject = ['typert', 'tools', 'llm', 'agents', 'subagents', 'sessionQuery', 'fs'];

export function apply(ctx) {
  // Boot diagnostics (best-effort): collected markers are written to
  // `dsh-agents-roles-boot.log` in the harness cwd. Two independent writers:
  // the fs service when it is up, and node:fs + process.cwd() directly (so a
  // missing fs service or a boot-order gap cannot hide the failure).
  const bootLog = [];
  const mark = (m) => { try { bootLog.push(String(m)); } catch (e) {} };
  try { mark('apply-start cwd=' + (typeof process !== 'undefined' ? process.cwd() : 'no-process')); } catch (e) {}
  try {
    applyBody(ctx, mark);
    mark('apply-end');
  } catch (e) {
    mark('FATAL: ' + (e && e.stack ? e.stack : String(e)));
  }
  const payload = bootLog.join('\n') + '\n';
  try {
    const fs = ctx.get('fs');
    if (fs && fs.resolve && fs.writeText) {
      fs.resolve('dsh-agents-roles-boot.log').then((p) => fs.writeText(p, payload)).catch(() => {});
    }
  } catch (e) { /* diagnostics are best-effort */ }
  try {
    import('node:fs').then((fsmod) => {
      const nfs = fsmod.default ?? fsmod;
      try { nfs.writeFileSync(process.cwd() + '/dsh-agents-roles-boot.log', payload); } catch (e) {}
    }).catch(() => {});
  } catch (e) { /* diagnostics are best-effort */ }
}

function applyBody(ctx, mark) {
  mark('apply-start');
  // ------------------------------------------------------------------
  ...
  // Optional integration service (workflows plugin may be absent): never
  // inject it, and treat a missing/unavailable service as "workflows off".
  ...
  mark('events-registered');

  // ------------------------------------------------------------------
  // Remote RPC service (`roles/*` endpoints over the /api channel).
  // The browser client calls them through connection.rpc.call("/api",
  // "roles/<method>", { args }) — see lib/client.js.
  //
  // The host Typert gateway dispatches a strict descriptor to the service it
  // names: the receiver must be reachable via ctx.get(serviceKey) and carry a
  // `typertRemote` binding { service, serviceKey, namespace }. A plain object
  // provided with ctx.provide satisfies both (no imports needed).
  // ------------------------------------------------------------------
  mark('typert-service=' + (ctx.get('typert') !== undefined ? 'present' : 'MISSING') + ' tools-service=' + (ctx.get('tools') !== undefined ? 'present' : 'MISSING'));
  const remote = {
    getState() {
      return snapshot();
    },
  ...
  remote.typertRemote = { service: remote, serviceKey: 'agentsRoles', namespace: 'roles' };

  // Mount the remote service + its `roles/*` invocation descriptors.
  try {
    ctx.provide('agentsRoles', remote);
    mark('provided-agentsRoles');
    const RPC_METHODS = ['getState', 'configure', 'getModelInfo', 'listModels', 'getStepModel', 'visionModel', 'getSessionRole', 'setSessionRole'];
    const invocations = RPC_METHODS.map((method) => {
      const needsArgs = method !== 'getState';
      return {
        id: 'dsh-agents-roles#roles/' + method,
        service: 'agentsRoles',
        namespace: 'roles',
        method,
        invocation: { kind: 'direct' },
        parameters: needsArgs
    ...
    ctx.typert.register({
      package: 'dsh-agents-roles',
      face: 'host',
      model: {},
      schemas: [],
      invocations,
    });
    mark('typert-registered=' + invocations.length + ' endpoints');
    console.log('[dsh-agents-roles] rpc handlers registered (roles/*)');
  } catch (e) {
    mark('RPC-ERROR: ' + (e && e.stack ? e.stack : String(e)));
    console.error('[dsh-agents-roles] rpc registration failed: ' + (e && e.message ? e.message : String(e)));
  }

  // ------------------------------------------------------------------
  // Model tools (roles_status / roles_configure / roles_spawn /
  // roles_assign), registered on the host tools service with the raw
  // registry-ready shape (parameters are already a JSON schema; no
  // defineTool import needed).
  // ------------------------------------------------------------------
  function registerTool(name, description, properties, required, execute) {
    try {
      ...
      mark('tool-registered=' + name);
      console.log('[dsh-agents-roles] tool registered: ' + name);
    } catch (e) {
      mark('TOOL-ERROR ' + name + ': ' + (e && e.stack ? e.stack : String(e)));
      console.error('[dsh-agents-roles] tool "' + name + '" registration failed: ' + (e && e.message ? e.message : String(e)));
    }
  }
  ...
  const summary = () => {
    ...
    lines.push('hint: roles_configure action=enable|set|disable|reset|show · config auto-persisted to ' + CONFIG_FILE);
  ...
  // fs may not be up on the very first ticks of host boot, so defer until
